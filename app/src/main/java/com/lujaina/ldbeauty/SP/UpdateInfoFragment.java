...
public class UpdateInfoFragment extends DialogFragment implements AdapterView.OnItemSelectedListener{
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View parentView = inflater.inflate(R.layout.fragment_update_info, container, false);
        ...
        if(mInfo != null){
            etTitle.setText(mInfo.getTitle());
            etBody.setText(mInfo.getBody());
        }
    ...
    public void setupdate(AddInfoModel info) {
        mInfo = info;
    }

    private void addInfoToDB(String title, String body) {
        FirebaseDatabase database = FirebaseDatabase.getInstance();
        DatabaseReference myRef = database.getReference(Constants.Users).child(Constants.Salon_Owner).child(mFirebaseUser.getUid()).child(Constants.Salon_Info).child(mInfo.getInfoId());
        about.setTitle(title);
        about.setBody(body);
        mUpdate.add(about);
        mAdapter.update(mUpdate.indexOf(about), about);
        myRef.child("title").setValue(about.getTitle());
        myRef.child("body").setValue(about.getBody());
        myRef.child("backgroundColor").setValue(about.getBackgroundColor()).addOnSuccessListener(new OnSuccessListener<Void>() {
            @Override
